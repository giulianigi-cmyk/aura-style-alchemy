// Microsoft redirects the browser here after the user approves (or
// denies) calendar access. Public route — no Supabase session is
// required or expected; the `state` value (created in
// startOutlookCalendarConnect, see calendar-connections.functions.ts)
// is what links this callback back to the AURA user who started the flow.
import { createFileRoute } from "@tanstack/react-router";

const APP_URL = "https://aura-wardrobe-intelligence.lovable.app";

export const Route = createFileRoute("/api/public/hooks/outlook-calendar-callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          ...
          await syncOutlookCalendar(userId);

          return Response.redirect(`${APP_URL}/?calendar=connected`, 302);
        } catch (err) {
          console.error("[AURA outlook-calendar] callback failed", err);
          return Response.redirect(`${APP_URL}/?calendar=error`, 302);
        }
      },
    },
